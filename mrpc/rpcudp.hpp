...
private:
	...
	std::vector<ulong64> _reqack();
	bool rpcudp::_accept_ack_impl(ulong64 id);
	void _check_ack(timer_handle, sysptr_t);
	void _timed_clear_ack(timer_handle handle, sysptr_t);
	...
	//一些操纵函数
	void _to_send(int flag,size_t group_id, const sockaddr_in& addr,
	...
	slice_vector _slice(const_memory_block,size_t keep);
	...
	//非洪水模式
	rwlock _recved_lock;
	std::hash_set<packid_type> _recved;
...
private:
	//
	//没有收到ack的包
	//std::hash_map<packid_type, resend_argument> _unack;
	//已经有过的包
	...
	//一些操纵函数
#define SEND_BY_UDPRPC 0x1
#define SEND_BY_SELF 0x10
